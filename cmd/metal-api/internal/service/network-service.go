...
func (r *networkResource) createNetwork(request *restful.Request, response *restful.Response) {
	...
	additionalRouteMapCIDRs, err := validateAdditionalRouteMapCIDRs(requestPayload.AdditionalRouteMapCIDRs, privateSuper)
	if err != nil {
		r.sendError(request, response, httperrors.BadRequest(err))
		return
	}
	...
	nw := &metal.Network{
		...
		AdditionalRouteMapCIDRs:  additionalRouteMapCIDRs,
	}
...
func (r *networkResource) updateNetwork(request *restful.Request, response *restful.Response) {
	...
	additionalRouteMapCIDRs, err := validateAdditionalRouteMapCIDRs(requestPayload.AdditionalRouteMapCIDRs, oldNetwork.PrivateSuper)
	if err != nil {
		r.sendError(request, response, defaultError(err))
		return
	}
	newNetwork.AdditionalRouteMapCIDRs = additionalRouteMapCIDRs
